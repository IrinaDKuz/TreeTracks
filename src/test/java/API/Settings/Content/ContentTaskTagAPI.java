...
/***
 ...
 во вкладке Settings - "Task Tag"
 */

public class ContentTaskTagAPI {
    public static int settingTagId;

    @Test
    public static void test() throws Exception {
        Allure.step("Добавляем TaskTag");
        ContentTag contentTagNew = tagAddEdit("add");
        Allure.step(CHECK);
        tagAssert(contentTagNew);
        Allure.step("Редактируем TaskTag");
        ContentTag contentTagEdit = tagAddEdit(settingTagId + "/edit");
        Allure.step(CHECK);
        tagAssert(contentTagEdit);
        deleteMethod("setting/task-tag", String.valueOf(settingTagId));
        assertDelete(String.valueOf(settingTagId), "task_tag");
    }
    ...
    public static ContentTag tagAddEdit(String method) {
        ...
        response = RestAssured.given()
                ...
                .post("https://api.admin.3tracks.link/setting/task-tag/" + method);
        ...
        if (method.equals("add")) {
            ...
            settingTagId = jsonResponse.getJSONObject("data").getInt("taskTag");
        } else
    ...
    public static ArrayList<ContentTag> tagGet() {
        ...
        Response response = RestAssured.given()
                ...
                .header("Accept", "application/json")
                .header("Content-Type", "application/json")
                .get("https://api.admin.3tracks.link/setting/task-tag");
        ...
        JSONArray dataArray = dataObject.getJSONArray("taskTag");
